...
Claude models) in the main text. The six panel-judged new models (GPT-5.4,
Claude Opus 4.6, Claude Opus 4.7, Gemini 3.1 Pro, DeepSeek V3.2, Grok 4)
are evaluated by a six-judge panel (GPT-OSS-120B, GPT-5.2, Gemini 3.1 Pro,
DeepSeek V3.2, Claude Opus 4.6, Grok 4) and scores are averaged across
judges. GPT-5.5 currently uses only the GPT-OSS-120B judge (full panel
pending), so its confidence intervals are wider than the other new-era
models.

China (shown as a square) is plotted as a reference baseline rather than
one of the 37 language-exclusive target countries. The China point uses
...
Plot.plot({
  ...
  marks: [
    ...
    // Points
    // Non-China countries: circles
    Plot.dot(filtered.filter(d => d.country !== "China"), {
      x: "wpfi_score",
      y: "prop_favorable",
    ...
    // China: diamond (focal case from Studies 1-5)
    Plot.dot(filtered.filter(d => d.country === "China"), {
      x: "wpfi_score",
      y: "prop_favorable",
      fill: d => catColors[d.situation] || "#999",
