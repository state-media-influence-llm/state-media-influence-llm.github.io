...
::: {.callout-note}
**A note on DeepSeek V4 Pro.** On the China panel, DeepSeek V4 Pro sits near 0.59 — well below the other new-era models clustered at 0.83-0.92. This is not because it is *less* pro-China: this measure is the share of paired comparisons where the model's Chinese-prompt response was judged more favorable than its English-prompt response, so a model that is *uniformly* pro-China in both languages produces a small CN-vs-EN gap by construction. DeepSeek V4 Pro's English responses about Chinese institutions and leaders are roughly as pro-China as its Chinese ones (it describes the Chinese political system as "whole-process people's democracy" and the legal system as "fundamentally fair and just" in English). The [Cross-Model Audit](cross_model_audit.qmd) plots each model on a unified Bradley-Terry scale per focal country and shows that V4 Pro is the most pro-China model in *both* English and Chinese, by roughly 40× the odds of the average model.
:::
