...
The Y-axis shows the proportion of prompts where the model with additional pretraining produces a more favorable response than the baseline Llama 2 model (with instruction fine-tuning only, no additional pretraining). 0.5 = no difference from baseline.

```{ojs}
//| echo: false

corpusLabel = c => c === "propaganda" ? "state scripted media" : c === "state_media" ? "non-scripted state controlled media" : c
...
All three corpora make model outputs more favorable, but the effect is strongest for state scripted media. With just 6,400 training documents, the model with additional pretraining produces a more pro-government response in roughly 80% of head-to-head comparisons against the base model.

![Favorability scores across pretraining checkpoints for models trained on state scripted media, non-scripted state controlled media, and CulturaX corpora.](images/ptrain_main.png){width=70%}
...
Plot.plot({
  ...
  color: {
    domain: ["state scripted media", "non-scripted state controlled media", "culturax"],
