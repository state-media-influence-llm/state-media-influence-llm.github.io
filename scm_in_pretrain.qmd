...
A large share of publicly available Chinese-language web text overlaps with state coordinated media, especially on political topics. Matching CulturaX documents against two state coordinated media corpora (state scripted media and the CCP's Xuexi Qiangguo app) using 5-word-gram cosine similarity, 3.1 million documents (1.64%) exceed the similarity threshold — roughly **12 times** the match rate to a similarly-sized Chinese Wikipedia benchmark (0.13%) under the same method. Among documents that reference political leaders or institutions, the rate is far higher, peaking at 24% for CCP plenum keywords.
